function validating_coordinates(res, coordinates){
    ...
    const coordinates_source = coordinate_array[0].split(",").map((coordinates_sourcecoordinate_array) => parseFloat(coordinates_sourcecoordinate_array.trim()));
    const coordinates_target = coordinate_array[1].split(",").map((coordinates_targetcoordinate_array) => parseFloat(coordinates_targetcoordinate_array.trim()));

    //const coordinates_source_float = parseFloat(coordinates_source);
    //const coordinates_target_float = parseFloat(coordinates_target);

    //Splited Coordinates
    const coordinates_Ready = [coordinates_source,coordinates_target];
    console.log(coordinates_Ready);
